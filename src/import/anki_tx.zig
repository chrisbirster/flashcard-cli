...
pub fn importCollection(
    ...
    const review_stmt = try prepare(source, "SELECT id, ease, type FROM revlog WHERE cid = ?1 AND ease BETWEEN 1 AND 4 AND NOT(type = 3 AND factor = 0) ORDER BY id;");
    defer _ = c.sqlite3_finalize(review_stmt);

    var rebuilds: std.ArrayList(RebuildTask) = .empty;
    defer rebuilds.deinit(allocator);
    var result: Result = .{};

    try destination.beginImmediate();
    errdefer destination.rollback();
    ...
    while (true) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                _ = try destination.appendReview(
                    destination_card_id,
                    try fsrs.Rating.fromValue(ease),
                    reviewed_at_ms,
                    null,
                    null,
                );
            ...
            if (last_review_ms) |timestamp| {
                try rebuilds.append(allocator, .{
                    .card_id = destination_card_id,
                    .last_reviewed_at_ms = timestamp,
                });
            }
        }
    }
    ...
    for (rebuilds.items) |task| {
        _ = try study.rebuildCardState(allocator, task.card_id, task.last_reviewed_at_ms);
    }
    return result;
}
